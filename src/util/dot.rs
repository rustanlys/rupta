...
{
    graph: G,
    node_fmt: &'a dyn Fn(&G::NodeWeight, &mut fmt::Formatter) -> fmt::Result,
    edge_fmt: &'a dyn Fn(&G::EdgeWeight, &mut fmt::Formatter) -> fmt::Result,
    ...
    G::NodeWeight: fmt::Display,
{
    ...
    pub fn with_attr_getters(
        ...
        Dot {
            graph,
            node_fmt: &fmt::Display::fmt,
            edge_fmt: &fmt::Display::fmt,
            get_edge_attributes,
            get_node_attributes,
            config,
        }
        ...
        for node in g.node_references() {
            ...
            if !self.config.NodeNoLabel {
                ...
                if self.config.NodeIndexLabel {
                    ...
                    Escaped(FnFmt(node.weight(), &self.node_fmt)).fmt(f)?;
                }
                write!(f, "\" ")?;
            }
        ...
        for (i, edge) in g.edge_references().enumerate() {
            ...
            if !self.config.EdgeNoLabel {
                ...
                if self.config.EdgeIndexLabel {
                    ...
                    Escaped(FnFmt(edge.weight(), &self.edge_fmt)).fmt(f)?;
                }
                write!(f, "\" ")?;
            }
